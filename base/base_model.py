...
from utils.plots import plot_dataset, plot_dataset3d, plot_samples
...
class BaseModel:
    ...
    ------------------------------------------------------------------------------ 
    '''
    def reconst_samples_from_data(self, data, session, cur_epoch=''):
        # Generating latent space
        print('Generating latent space ...')
        latent_en = self.encode(data.x)

        pca = PCA(n_components=2)
        latent_pca = pca.fit_transform(latent_en)
        print('Latent space dimensions: {}'.format(latent_pca.shape))
        print('Plotting latent space ...')
        latent_space = self.config.log_dir + '/{} latent epoch {}.jpg'.format(self.config.log_dir.split('/')[-1:][0],
                                                                          cur_epoch)
        ...
        recons_file = self.config.log_dir + '/{} samples reconstruction epoch {}.jpg'.format(
