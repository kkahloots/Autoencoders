...
'''
class AnnealedVAEGraph(BaseGraph):
    ...
    def train_epoch(self, session, x):
        tensors = [self.train_step,  self.annvae_loss, self.vae_loss, self.ae_loss, self.loss_reconstruction_m, self.L2_loss]
    ...
    def evaluate_epoch(self, session, x):
        tensors = [self.annvae_loss, self.vae_loss, self.ae_loss, self.loss_reconstruction_m, self.L2_loss]
        feed_dict = {self.x_batch: x}
